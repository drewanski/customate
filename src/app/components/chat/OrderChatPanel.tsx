...
export function OrderChatPanel({
  ...
  // Browser confirm/prompt looks unprofessional and on some browsers
  // ignores keyboard focus traps. Real modals improve UX + show the
  // customer's payment context (amount, method, ref) right next to the
  // decision so admin doesn't have to scroll.
  const [verifyTarget, setVerifyTarget] = useState<{ stage: 'downpayment' | 'balance'; amount: number; method?: string; reference?: string } | null>(null);
  const [verifyBusy, setVerifyBusy] = useState(false);
  const [rejectTarget, setRejectTarget] = useState<{ stage: 'downpayment' | 'balance'; amount: number } | null>(null);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col shadow-sm">
                          ...
                          <div className="p-4 space-y-2">
                            ...
                            {myRole === 'admin' && (
                              <div className="flex gap-2 pt-1">
                                <button
                                  onClick={() => setVerifyTarget({ stage, amount: Number(amount), method: m.meta.method, reference: m.meta.reference })}
                                ...
                                <button
                                  onClick={() => { setRejectReason(''); setRejectTarget({ stage, amount: Number(amount) }); }}
              ...
              <div className="mb-2.5 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 text-white font-bold text-xs shadow-md hover:shadow-lg ${hasLink ? '' : 'col-span-2'}`}
                  >
      ...
      {/* ── ADMIN — Verify payment proof confirmation modal ────────── */}
      {verifyTarget && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => !verifyBusy && setVerifyTarget(null)}>
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-4 bg-gradient-to-br from-emerald-500 to-teal-600 text-white">
              <h3 className="text-lg font-black tracking-tight inline-flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5" /> Verify {verifyTarget.stage} payment
              </h3>
              <p className="text-sm opacity-90 mt-0.5">Confirm the customer's payment is real before marking it verified.</p>
            </div>
            <div className="p-5 space-y-3">
              <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-slate-600">Stage</span><span className="font-bold text-slate-900 capitalize">{verifyTarget.stage}</span></div>
                <div className="flex justify-between"><span className="text-slate-600">Amount</span><span className="font-black text-emerald-700">₱{verifyTarget.amount.toLocaleString()}</span></div>
                {verifyTarget.method && <div className="flex justify-between"><span className="text-slate-600">Method</span><span className="font-semibold capitalize text-slate-900">{verifyTarget.method}</span></div>}
                {verifyTarget.reference && <div className="flex justify-between"><span className="text-slate-600">Reference</span><code className="font-mono text-xs bg-white border border-slate-200 px-1.5 py-0.5 rounded">{verifyTarget.reference}</code></div>}
              </div>
              <p className="text-xs text-slate-500 leading-snug">
                Click <strong>Verify</strong> only after confirming the payment landed in your account (GCash inbox / bank notification / PayMongo dashboard).
                {verifyTarget.stage === 'downpayment' ? ' This will move the order to ' : ' This will mark the order as fully paid and unlock '}
                <strong>{verifyTarget.stage === 'downpayment' ? '"Downpayment Verified"' : 'release for delivery / pickup'}</strong>.
              </p>
              <div className="flex gap-2 pt-1">
                <button onClick={() => setVerifyTarget(null)} disabled={verifyBusy} className="flex-1 px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm disabled:opacity-60">Cancel</button>
                <button
                  onClick={async () => {
                    setVerifyBusy(true);
                    try {
                      await verifyPayment(orderId, verifyTarget.stage);
                      window.location.reload();
                    } catch (e: any) {
                  ...
                  disabled={verifyBusy}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-700 text-white font-black text-sm shadow-md disabled:opacity-60"
                >
                  {verifyBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
      ...
      {/* ── ADMIN — Reject payment proof modal (with reason textarea) ── */}
      {rejectTarget && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => !rejectBusy && setRejectTarget(null)}>
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-4 bg-gradient-to-br from-rose-500 to-red-600 text-white">
              <h3 className="text-lg font-black tracking-tight inline-flex items-center gap-2">
                <XCircle className="w-5 h-5" /> Reject {rejectTarget.stage} proof
              </h3>
              <p className="text-sm opacity-90 mt-0.5">The customer will see your reason and can re-upload a corrected screenshot.</p>
            </div>
            <div className="p-5 space-y-3">
              <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-sm">
                <p><span className="text-slate-600">Amount: </span><span className="font-black text-slate-900">₱{rejectTarget.amount.toLocaleString()}</span></p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">Reason for rejection (shown to customer)</label>
                <textarea
                  value={rejectReason}
                  onChange={(e) => setRejectReason(e.target.value)}
                  rows={3}
                  placeholder="e.g. Amount doesn't match (you sent ₱200, owed ₱335). Please re-upload."
                ...
                />
                {!rejectReason.trim() && <p className="text-[11px] text-rose-600 mt-1">A reason is required.</p>}
              </div>
              <div className="flex gap-2 pt-1">
                <button onClick={() => setRejectTarget(null)} disabled={rejectBusy} className="flex-1 px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm disabled:opacity-60">Cancel</button>
                <button
                  onClick={async () => {
                    if (!rejectReason.trim()) return;
                    setRejectBusy(true);
                    try {
                      await rejectPayment(orderId, rejectTarget.stage, rejectReason.trim());
                      window.location.reload();
                    } catch (e: any) {
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-rose-600 text-white font-black text-sm shadow-md disabled:opacity-50"
                >
